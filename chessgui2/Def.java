/**
	@author Jinwook Shin 100227640
	Date finished : 6/24/19 (Mon)
*/

/**
 * 
 * Def class contains all the static variables used for the chess.
 */

public class Def {
	// Colors:
	public static final int NO_COLOR = 1;
	public static final int BLACK = 2;
	public static final int WHITE = 3;
	public static final int LENGTH = 60;
	public static final int GAP = 30;

	// Pieces:
	public static final int PAWN = 4;
	public static final int KNIGHT = 5;
	public static final int BISHOP = 6;
	public static final int ROOK = 7;
	public static final int QUEEN = 8;
	public static final int KING = 9;
	public static final int BOARDER = 10;
	public static final int DUMMY = 11;

	// Symbols

	public static final String PAWN_BLACK = "image/pawn_black.png";
	public static final String PAWN_WHITE = "image/pawn_white.png";
	public static final String QUEEN_BLACK = "image/queen_black.png";
	public static final String QUEEN_WHITE = "image/queen_white.png";
	public static final String KING_BLACK = "image/king_black.png";
	public static final String KING_WHITE = "image/king_white.png";
	public static final String ROOK_BLACK = "image/rook_black.png";
	public static final String ROOK_WHITE = "image/rook_white.png";
	public static final String KNIGHT_BLACK = "image/knight_black.png";
	public static final String KNIGHT_WHITE = "image/knight_white.png";
	public static final String BISHOP_BLACK = "image/bishop_black.png";
	public static final String BISHOP_WHITE = "image/bishop_white.png";

}
